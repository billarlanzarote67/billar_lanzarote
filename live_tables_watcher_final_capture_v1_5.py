...
import os, json, time, sqlite3, re, unicodedata, requests
from datetime import datetime

ROOT = r"C:\AI\BillarLanzarote"
CFG_PATH = os.path.join(ROOT, "config", "live_tables_watcher_final_capture_config_v1_5.json")
STATE_DIR = os.path.join(ROOT, "state")
DATA_DIR = os.path.join(ROOT, "data")
COMPLETED_DIR = os.path.join(DATA_DIR, "completed_matches")
DB_PATH = os.path.join(DATA_DIR, "billar_lanzarote.sqlite3")
LOG_PATH = os.path.join(ROOT, "logs", "live_tables_watcher_final_capture_v1_5.log")
FINAL_STATE_PATH = os.path.join(STATE_DIR, "final_capture_state_v1_5.json")
...
def init_db():
    ...
    for stmt in [
        "ALTER TABLE matches_final ADD COLUMN stats_status TEXT",
        "ALTER TABLE matches_final ADD COLUMN table_display_name_es TEXT",
        "ALTER TABLE matches_final ADD COLUMN game_type_es TEXT",
        "ALTER TABLE latest_table_state ADD COLUMN table_display_name_es TEXT",
        "ALTER TABLE latest_table_state ADD COLUMN game_type_es TEXT",
        "ALTER TABLE live_table_events ADD COLUMN table_display_name_es TEXT",
        "ALTER TABLE live_table_events ADD COLUMN game_type_es TEXT",
    ]:
        try:
            cur.execute(stmt)
    ...
    """)
    try:
        cur.execute("ALTER TABLE player_match_history ADD COLUMN game_type_es TEXT")
    except sqlite3.OperationalError:
        pass
    try:
        cur.execute("ALTER TABLE player_match_history ADD COLUMN stats_status TEXT")
    except sqlite3.OperationalError:
        pass
...
def detect_game_type_es(text):
    if not text:
        return None
    cleaned = strip_flags_and_symbols(text)
...
def extract_player_name(block_text):
    ...
    for line in block_text.splitlines():
        ...
        if any(phrase in low for phrase in JUNK_NAME_PHRASES):
            continue
        if low in {"10-ball","9-ball","8-ball","bola 10","bola 9","bola 8","challengematch","match statistics","statistics"}:
            continue
        if len(line) < 2:
            continue
        lines.append(line)
    if not lines:
        return None
    return re.sub(r"\s+", " ", " ".join(lines[:2])).strip(" -|")

def write_name_debug(st):
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_json(os.path.join(RAW_NAME_DEBUG_DIR, f"{st['table_key']}_{ts}.json"), st)
...
def parse_table(page, table_key, meta):
    ...
    game_type_es = detect_game_type_es(body_text)

    names_ok = bool(player_a and player_b)
    scores_ok = score_a is not None and score_b is not None
    race_ok = race_to is not None
    ...
    winner = None
    loser = None
    if names_ok and scores_ok and race_ok:
        if score_a >= race_to:
            winner = player_a
            loser = player_b
    ...
    state = {
        ...
        "table_display_name_es": meta.get("display_name_es", meta.get("name", table_key)),
        "game_type_es": game_type_es,
        "status": status,
        "player_a": player_a,
        "player_b": player_b,
        ...
        "score_a": score_a,
        "score_b": score_b,
        "score": f"{score_a}-{score_b}" if scores_ok else None,
        "race_to": race_to,
        "race_text_es": f"Carrera a {race_to}" if race_ok else "Carrera desconocida",
        "winner": winner,
        "loser": loser,
...
def parse_final_stats_from_body(body_text):
    ...
    if any(v is not None for k, v in result.items() if k not in {"stats_status","raw_stats_text"}):
        result["stats_status"] = "captured"
    return result
...
def write_live_state(conn, st):
    ...
    cur.execute("""
        INSERT INTO live_table_events (
            table_key, table_name, table_display_name_es, game_type_es, status, player_a, player_b, score_a, score_b,
            score, race_to, race_text_es, winner, loser, source_url, raw_player_a, raw_player_b
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        st.get("table_key"), st.get("table_name"), st.get("table_display_name_es"), st.get("game_type_es"),
        st.get("status"), st.get("player_a"), st.get("player_b"), st.get("score_a"), st.get("score_b"),
        st.get("score"), st.get("race_to"), st.get("race_text_es"), st.get("winner"), st.get("loser"),
    ...
    cur.execute("""
        ...
        ON CONFLICT(table_key) DO UPDATE SET
            ...
            table_display_name_es=excluded.table_display_name_es,
...
def save_final_match(conn, st, stats):
    ...
    safe_game = re.sub(r"[^A-Za-z0-9._ -]+", "", (st.get("game_type_es") or "SinJuego")).replace(" ", "_")
    json_path = os.path.join(COMPLETED_DIR, f"{st['table_key']}_{safe_game}_{ts}_{safe_a}_vs_{safe_b}.json")
    ...
    cur.execute("""
        INSERT INTO matches_final (
            match_signature, table_key, table_name, table_display_name_es, game_type_es, player_a, player_b,
            score_a, score_b, score, race_to, winner, loser, source_url, raw_player_a, raw_player_b,
            stats_status, avg_frametime_a, avg_frametime_b, win_pct_a, win_pct_b, runouts_a, runouts_b,
            break_and_wins_a, break_and_wins_b, frames_stolen_a, frames_stolen_b, timeouts_a,
            timeouts_b, raw_stats_text, raw_json_path
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        signature, st.get("table_key"), st.get("table_name"), st.get("table_display_name_es"), st.get("game_type_es"),
        st.get("player_a"), st.get("player_b"), st.get("score_a"), st.get("score_b"), st.get("score"),
        st.get("race_to"), st.get("winner"), st.get("loser"), st.get("source_url"),
        st.get("raw_player_a"), st.get("raw_player_b"), stats.get("stats_status"),
        stats.get("avg_frametime_a"), stats.get("avg_frametime_b"),
        stats.get("win_pct_a"), stats.get("win_pct_b"), stats.get("runouts_a"), stats.get("runouts_b"),
        stats.get("break_and_wins_a"), stats.get("break_and_wins_b"), stats.get("frames_stolen_a"), stats.get("frames_stolen_b"),
        stats.get("timeouts_a"), stats.get("timeouts_b"), stats.get("raw_stats_text"), json_path
    ))

    cur.executemany("""
        INSERT INTO player_match_history (
            match_signature, player_name, opponent_name, table_key, game_type_es, did_win, frames_won, frames_lost,
            winner, avg_frametime, runouts, break_and_wins, frames_stolen, timeouts, stats_status
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, [
        (
            signature, st.get("player_a"), st.get("player_b"), st.get("table_key"), st.get("game_type_es"),
            ...
            stats.get("frames_stolen_a"), stats.get("timeouts_a"), stats.get("stats_status")
        ),
        (
            signature, st.get("player_b"), st.get("player_a"), st.get("table_key"), st.get("game_type_es"),
            ...
            stats.get("frames_stolen_b"), stats.get("timeouts_b"), stats.get("stats_status")
        ),
    ])
    conn.commit()
    ...
    return True, json_path, signature

def update_saved_match_with_stats(conn, signature, stats, json_path):
    cur = conn.cursor()
    cur.execute("""
        UPDATE matches_final SET
            stats_status=?,
            avg_frametime_a=?,
            avg_frametime_b=?,
            win_pct_a=?,
            ...
            timeouts_b=?,
            raw_stats_text=?
        WHERE match_signature=?
    """, (
        stats.get("stats_status"), stats.get("avg_frametime_a"), stats.get("avg_frametime_b"),
        stats.get("win_pct_a"), stats.get("win_pct_b"), stats.get("runouts_a"), stats.get("runouts_b"),
        stats.get("break_and_wins_a"), stats.get("break_and_wins_b"), stats.get("frames_stolen_a"), stats.get("frames_stolen_b"),
        stats.get("timeouts_a"), stats.get("timeouts_b"), stats.get("raw_stats_text"), signature
    ))
    conn.commit()
...
def try_capture_stats_after_finish(page, wait_seconds=5):
    deadline = time.time() + wait_seconds
    best = {"stats_status": "pending", "raw_stats_text": None,
            "avg_frametime_a": None, "avg_frametime_b": None,
            "win_pct_a": None, "win_pct_b": None,
            "runouts_a": None, "runouts_b": None,
            "break_and_wins_a": None, "break_and_wins_b": None,
            "frames_stolen_a": None, "frames_stolen_b": None,
            "timeouts_a": None, "timeouts_b": None}
    while time.time() < deadline:
        stats = parse_final_stats_from_body(get_body_text(page))
        if stats.get("stats_status") == "captured":
            return stats
        best = stats
        time.sleep(1)
    return best
                    ...
                    try:
                        ...
                        if last_digest.get(table_key) != digest:
                            if state.get("status") == "live":
                                ...
                                log(f"{table_key}: live -> {state.get('player_a')} {state.get('score') or 'no-score'} {state.get('player_b')} | {state.get('game_type_es') or 'Sin juego'}")
                            last_digest[table_key] = digest

                        if state.get("status") == "live" and state.get("race_to") is not None and state.get("winner"):
                            signature = make_signature(state)
                            if last_saved_signature.get(table_key) != signature:
                                locked_game = state.get("game_type_es")
                                if not locked_game:
                                    locked_game = detect_game_type_es(get_body_text(page))
                                state["game_type_es"] = locked_game
                                ...
                                saved, json_path, signature = save_final_match(conn, state, initial_stats)
                                if saved:
                                    last_saved_signature[table_key] = signature
                                    log(f"{table_key}: FINAL SAVED immediately -> {json_path}")
                                    stats = try_capture_stats_after_finish(page, wait_seconds=5)
                                    update_saved_match_with_stats(conn, signature, stats, json_path)
                                    log(f"{table_key}: stats status -> {stats.get('stats_status')}")
                                    msg = (
                                        f"🏆 Final {state.get('table_display_name_es')}\n"
                                        f"Juego: {state.get('game_type_es') or 'Sin juego'}\n"
                                        f"{state.get('player_a') or '—'} {state.get('score') or ''} {state.get('player_b') or '—'}\n"
                                        f"Ganador: {state.get('winner') or '—'}\n"
                                        f"Stats: {stats.get('stats_status') or 'pending'}"
                                    )
