...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/":
            ...
            body += "<div class='card'><h2>Partidas recientes</h2><table><tr><th>ID</th><th>Mesa</th><th>Juego</th><th>Jugador A</th><th>Marcador</th><th>Jugador B</th><th>Ganador</th><th>Stats extra</th><th>Guardado</th></tr>"
            for r in matches:
                extras = (
                ...
                body += (
                    f"<tr>"
                    f"<td>{r['id']}</td>"
                    f"<td>{html.escape(str(r['table_display_name_es'] or ''))}</td>"
                    f"<td>{html.escape(str(r['game_type_es'] or ''))}</td>"
        ...
        if parsed.path == "/player":
            ...
            rows = fetch_all("""
                SELECT id, opponent_name, table_key, game_type_es, did_win, frames_won, frames_lost,
                       winner, avg_frametime, runouts, break_and_wins, frames_stolen, timeouts, created_ts_utc
            ...
            if prof:
                ...
                body += (
                    "<div class='card'>"
                    f"<b>Jugadas:</b> {p['matches_played']} &nbsp; "
                    f"<b>Ganadas:</b> <span class='good'>{p['matches_won']}</span> &nbsp; "
                    f"<b>Perdidas:</b> <span class='bad'>{p['matches_lost']}</span> &nbsp; "
                    f"<b>Frames G:</b> {p['frames_won']} &nbsp; "
                    f"<b>Frames P:</b> {p['frames_lost']} &nbsp; "
                    f"<b>Win %:</b> {p['win_pct']} &nbsp; "
                    f"<div class='small'>Actualizado: {html.escape(str(p['updated_ts_utc'] or ''))}</div>"
                    "</div>"
                )

            body += "<div class='card'><h2>Historial</h2><table><tr><th>ID</th><th>Rival</th><th>Mesa</th><th>Juego</th><th>Resultado</th><th>Frames</th><th>Tiempo medio</th><th>Runouts</th><th>B&W</th><th>Robadas</th><th>Tiempos</th><th>Creado</th></tr>"
            for r in rows:
