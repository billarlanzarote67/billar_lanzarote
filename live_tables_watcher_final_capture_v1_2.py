...
CFG_PATH = os.path.join(ROOT, "config", "live_tables_watcher_final_capture_config_v1_2.json")
STATE_DIR = os.path.join(ROOT, "state")
DATA_DIR = os.path.join(ROOT, "data")
COMPLETED_DIR = os.path.join(DATA_DIR, "completed_matches")
DB_PATH = os.path.join(DATA_DIR, "billar_lanzarote.sqlite3")
LOG_PATH = os.path.join(ROOT, "logs", "live_tables_watcher_final_capture_v1_2.log")
FINAL_STATE_PATH = os.path.join(STATE_DIR, "final_capture_state_v1_2.json")
...
def extract_player_name(block_text):
    ...
    # Keep only the first 2 sensible lines max.
    # This avoids the stats block being dragged in.
    lines = lines[:2]
    ...
    low_name = name.lower()
    if low_name in bad_exact:
        return None
    if any(phrase == low_name for phrase in bad_phrases):
        return None
...
def parse_table(page, table_key, meta):
    ...
    bad_names = {"breaking", "runouts", "runout", "rack", "undo"}
    if player_a and player_a.lower() in bad_names:
        player_a = None
    if player_b and player_b.lower() in bad_names:
        player_b = None
...
def save_final_match(conn, st):
    ...
    filename = f"{st['table_key']}_{ts}_{safe_a}_vs_{safe_b}.json"
    json_path = os.path.join(COMPLETED_DIR, filename)
    ...
    cur.execute("""
        INSERT INTO matches_final (
            ...
            score_a, score_b, score, race_to, winner, loser, source_url, raw_player_a, raw_player_b, raw_json_path
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        signature,
        st.get("table_key"),
                ...
                for table_key, meta in cfg["tables"].items():
                    ...
                    try:
                        ...
                        if last_digest.get(table_key) != digest:
                            ...
                            log(f"{table_key}: state changed -> {state.get('player_a')} {state.get('score') or 'no-score'} {state.get('player_b')}")
                        ...
                        if state.get("winner") and state.get("race_to") is not None:
                            ...
                            if current["count"] >= stable_needed:
                                saved, _, json_path = save_final_match(conn, state)
                                if saved:
                                    msg = (
