...
import os, json, time, sqlite3, re, unicodedata
from datetime import datetime
import requests

ROOT = r"C:\AI\BillarLanzarote"
CFG_PATH = os.path.join(ROOT, "config", "live_tables_watcher_final_capture_config_v1_4.json")
STATE_DIR = os.path.join(ROOT, "state")
DATA_DIR = os.path.join(ROOT, "data")
COMPLETED_DIR = os.path.join(DATA_DIR, "completed_matches")
DB_PATH = os.path.join(DATA_DIR, "billar_lanzarote.sqlite3")
LOG_PATH = os.path.join(ROOT, "logs", "live_tables_watcher_final_capture_v1_4.log")
FINAL_STATE_PATH = os.path.join(STATE_DIR, "final_capture_state_v1_4.json")
...
JUNK_EXACT = {
    ...
    "8-ball", "challengematch", "match statistics", "statistics", "bola 8", "bola 9"
}
...
def parse_float_percent(value):
    if value is None:
        return None
    txt = str(value).replace("%", "").strip().replace(",", ".")
    try:
        return float(txt)
    except Exception:
        return None
...
def write_name_debug(st):
    ...
    save_json(os.path.join(RAW_NAME_DEBUG_DIR, f"{st['table_key']}_{ts}.json"), {
        "table_key": st.get("table_key"),
        "table_name": st.get("table_name"),
        "table_display_name_es": st.get("table_display_name_es"),
        "game_type_es": st.get("game_type_es"),
...
def save_final_match(conn, st, stats):
    ...
    cur.execute("""
        INSERT INTO matches_final (
            ...
            avg_frametime_a, avg_frametime_b, win_pct_a, win_pct_b, runouts_a, runouts_b,
                ...
                for table_key, meta in cfg["tables"].items():
                    ...
                    try:
                        ...
                        if last_digest.get(table_key) != digest:
                            ...
                            log(f"{table_key}: state changed -> {state.get('player_a') or '—'} {state.get('score') or 'no-score'} {state.get('player_b') or '—'} | {state.get('game_type_es') or 'Sin juego'}")
